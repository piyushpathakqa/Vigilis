# Vigilis — Product & Engineering Context
...
Vigilis is a **trust and provenance layer for autonomous testing** — not "another self-healing test tool."

The thesis: an agent rewrites tests autonomously, so the only question that matters is **"can I trust what it did?"** Vigilis answers that with verifiable, signed proof of every tool call and model decision. Self-healing is the *wedge* (it earns adoption); attestation is the *product* (it's the defensible value).
...
4. **Agent-agnostic.** Where possible, sit *on top of* execution/healing agents and attest their actions rather than reinventing the healer. The long-term target is attesting any engineering agent, not just Vigilis's own.
...
4. **Independent notary.** The provider (Treeship) is a *separate* system from Vigilis. This independence is what makes the proof credible to third parties — Vigilis signing its own logs would be "marking its own homework."
...
- **Delegate key management and crypto to the provider.** Do not roll custom crypto or key handling inside Vigilis.
...
- **TestDino:** observability/analytics over test reliability — mutable dashboards, not cryptographic proof. Vigilis's edge is verifiable evidence.
- **Horizontal attestation (e.g. Prova):** general agent-attestation category. Vigilis's defensibility is QA-vertical depth + integration, not "owning attestation."
...
- Custom crypto or key management inside Vigilis.
